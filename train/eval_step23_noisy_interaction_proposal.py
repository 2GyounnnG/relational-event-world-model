...
def compare_metric_groups(candidate: Dict[str, Any], baseline: Dict[str, Any]) -> Dict[str, Any]:
    return {
        key: delta(candidate.get(key), baseline.get(key))
...
def main() -> None:
    ...
    parser.add_argument("--candidate_rft1_run_name", type=str, default="noisy_step23_p2_rft1")
    parser.add_argument("--candidate_w012_run_name", type=str, default="noisy_step23_p2_w012")
    ...
    run_paths = {
        ...
        "step23_noisy_interaction_p2_rft1": step23_dir / f"{args.candidate_rft1_run_name}.json",
        "step23_noisy_interaction_p2_w012_optional": step23_dir / f"{args.candidate_w012_run_name}.json",
    }
    ...
    candidate = runs.get("step23_noisy_interaction_p2_rft1")
    baseline = runs.get("baseline_noisy_p2_rft1")
    if candidate is not None and baseline is not None:
        comparisons["step23_rft1_minus_p2_rft1"] = {
    ...
    summary = {
        "metadata": {
            "evaluation": "Step 23 noisy interaction-aware proposal comparison",
            "single_mechanism_variable": "proposal_training_regime = baseline_noisy_P2 | noisy_interaction_aware_P2",
            ...
            "note": "This script consolidates Step 22/23 evaluator outputs; it does not run inference itself.",
        },
    ...
    out_json = step23_dir / f"{args.output_name}.json"
    out_csv = step23_dir / f"{args.output_name}.csv"
