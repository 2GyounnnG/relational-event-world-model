...
def main() -> None:
    ...
    parser.add_argument("--step28_run_name", type=str, default="noisy_step28_rft1_anchored_joint")
    ...
    run_paths = {
        ...
        "step26_coverage_joint": step26_dir / "noisy_step26_joint_deeper.json",
        "step26_proposal_rft1_factorized": step27_dir / "step26_proposal_baseline_rewrite.json",
        "step28_rft1_anchored_joint": step28_dir / f"{args.step28_run_name}.json",
        "oracle_rft1_reference_optional": PROJECT_ROOT / "artifacts/step25_noisy_multievent_oracle_headroom/oracle_scope_rft1.json",
    }
    ...
    candidate = runs.get("step28_rft1_anchored_joint")
    comparisons: Dict[str, Any] = {}
    add_comparison(comparisons, "step28_minus_baseline_noisy_p2_rft1", candidate, runs.get("baseline_noisy_p2_rft1"))
    add_comparison(comparisons, "step28_minus_step24_light_joint", candidate, runs.get("step24_light_joint"))
    add_comparison(comparisons, "step28_minus_step26_coverage_joint", candidate, runs.get("step26_coverage_joint"))
    add_comparison(
        comparisons,
        "step28_minus_step26_proposal_rft1_factorized",
        candidate,
        runs.get("step26_proposal_rft1_factorized"),
    )
    add_comparison(
        comparisons,
        "step28_minus_noisy_p2_i1520_optional",
        candidate,
        runs.get("baseline_noisy_p2_i1520_optional"),
    )

    summary = {
        "metadata": {
            "evaluation": "Step 28 RFT1-anchored coverage-emphasized joint comparison",
            "single_mechanism_variable": "rewrite_constraint_regime = none | rft1_anchored_joint",
            "fixed_thresholds": {"node_threshold": 0.15, "edge_threshold": 0.10},
            "recipe_note": (
                "Step28 keeps Step26 proposal coverage pressure and adds one rewrite parameter "
                "anchor toward the RFT1 initialization."
            ),
            "step28_artifact_dir": str(step28_dir),
        },
    ...
    out_json = step28_dir / f"{args.output_name}.json"
    out_csv = step28_dir / f"{args.output_name}.csv"
