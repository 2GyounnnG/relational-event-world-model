...
from train.train_step21_two_path_local_subgraph_chooser import (
    LEARNED_TWO_PATH_LOCAL_SUBGRAPH,
    build_step21_feature_tensors,
    load_step21_chooser_model,
)


STEP21_PREFIX = "step21_two_path_local_subgraph"


def mode_name(frac: float) -> str:
    return f"{STEP21_PREFIX}_topk_keep_{format_keep_fraction(frac)}"
...
def compute_global_thresholds(
    ...
    for frac in keep_fractions:
        ...
        thresholds[frac] = float("inf") if keep_count <= 0 else sorted_scores[min(keep_count, scores.numel()) - 1].item()
    return thresholds
...
def evaluate_step21(
    ...
    all_modes = [mode_name(frac) for frac in KEEP_FRACTIONS]
    ...
    for batch in loader:
        ...
        for frac in KEEP_FRACTIONS:
            mode = mode_name(frac)
            choose_mask = (scores >= keep_score_thresholds[frac]) & rescued & valid_edge_mask
        ...
        for sample_idx in range(batch["node_feats"].shape[0]):
            ...
            for group in groups:
                update_rank_bucket(rank_buckets[group], scores, target_choose_step, rescued, valid_edge_mask, sample_idx)
    ...
    ranking = format_grouped_results(
        {k: finalize_rank_bucket(v) for k, v in rank_buckets.items()},
        finalize_rank_bucket(init_rank_bucket()),
    )
    return {"downstream": downstream, "decomposition": decomp, "chooser_behavior": chooser, "ranking_diagnostics": {STEP21_PREFIX: ranking}}


def merge_with_reference(reference: Dict[str, Any], step21: Dict[str, Any]) -> Dict[str, Any]:
    merged = copy.deepcopy(reference["results"])
    for section in ["downstream", "decomposition", "chooser_behavior"]:
        merged.setdefault(section, {}).update(step21.get(section, {}))
    merged.setdefault("ranking_diagnostics", {}).update(step21["ranking_diagnostics"])
...
def main() -> None:
    ...
    parser.add_argument("--chooser_checkpoint_path", type=str, required=True)
    parser.add_argument("--reference_step20_json_path", type=str, required=True)
    parser.add_argument("--data_path", type=str, required=True)
    parser.add_argument("--split_name", type=str, default="test")
    parser.add_argument("--run_name", type=str, default="")
    parser.add_argument("--artifact_dir", type=str, default="artifacts/step21_two_path_local_subgraph_chooser")
    ...
    chooser_checkpoint_path = resolve_path(args.chooser_checkpoint_path)
    reference_step20_json_path = resolve_path(args.reference_step20_json_path)
    data_path = resolve_path(args.data_path)
    artifact_dir = resolve_path(args.artifact_dir)

    reference_payload = json.load(open(reference_step20_json_path, encoding="utf-8"))
    ...
    chooser_model = load_step21_chooser_model(chooser_checkpoint_path, device)
    ...
    step21_results = evaluate_step21(
        proposal_model=proposal_model,
        completion_model=completion_model,
        rewrite_model=rewrite_model,
        chooser_model=chooser_model,
        ...
        edge_threshold=args.edge_threshold,
        use_proposal_conditioning=use_proposal_conditioning,
        keep_score_thresholds=thresholds,
    )
    results = merge_with_reference(reference_payload, step21_results)
    ...
    payload = {
        "metadata": {
            ...
            "chooser_checkpoint_path": str(chooser_checkpoint_path),
            "reference_step20_json_path": str(reference_step20_json_path),
            ...
            "step21_keep_score_thresholds": {str(frac): value for frac, value in thresholds.items()},
        },
        "results": results,
    }
