...
#   python train/train_oracle_local_delta_proposal_conditioned_fp_keep.py
# ---------------------------------------------------------------------
...
from models.oracle_local_delta import (
    ...
    masked_edge_delete_regularization_loss_from_pair_weights,
    masked_edge_keep_regularization_loss_from_pair_mask,
    masked_edge_keep_regularization_loss_from_pair_weights,
...
def init_metric_accumulator() -> Dict[str, float]:
    return {
        ...
        "local_fp_edge_keep_loss_sum": 0.0,
...
def finalize_metric_accumulator(acc: Dict[str, float]) -> Dict[str, float]:
    ...
    return {
        ...
        "local_fp_edge_keep_loss": acc["local_fp_edge_keep_loss_sum"] / num_batches,
...
def update_metric_accumulator(
    ...
    acc["local_fp_edge_keep_loss_sum"] += local_loss_dict["fp_edge_keep_loss"].item()
...
def run_oracle_epoch(
    ...
    fp_keep_lowconf_only: bool,
    fp_keep_lowconf_upper: float,
    fp_keep_weighted: bool,
    fp_keep_weighted_upper: float,
    fp_keep_asymmetric: bool,
    fp_keep_absent_weight: float,
    fp_keep_present_weight: float,
    use_oracle_delete_rescue: bool,
    delete_rescue_weight: float,
    delete_rescue_upper: float,
    node_threshold: float,
    edge_threshold: float,
    ...
    with context:
        for batch in loader:
            ...
            edge_delta_targets = build_edge_delta_targets(current_adj, batch["next_adj"])
            predicted_only_edge_mask = pred_scope_edges * valid_edge_mask * (1.0 - oracle_edge_scope_mask)
            if fp_keep_asymmetric:
                predicted_only_absent_mask = predicted_only_edge_mask * (1.0 - current_adj.float())
                predicted_only_present_mask = predicted_only_edge_mask * current_adj.float()
                absent_keep_loss = masked_edge_keep_regularization_loss_from_pair_mask(
                    outputs["edge_delta_logits_local"],
                    predicted_only_absent_mask,
                )
                present_keep_loss = masked_edge_keep_regularization_loss_from_pair_mask(
                    outputs["edge_delta_logits_local"],
                    predicted_only_present_mask,
                )
                fp_edge_keep_loss = (
                    fp_keep_absent_weight * absent_keep_loss
                    + fp_keep_present_weight * present_keep_loss
                )
                total_loss = local_loss_dict["total_loss"] + fp_edge_keep_loss
            elif fp_keep_weighted:
                upper = fp_keep_weighted_upper
                if upper <= edge_threshold:
                    predicted_only_edge_weights = predicted_only_edge_mask * 0.0
                else:
                    predicted_only_edge_weights = predicted_only_edge_mask * torch.clamp(
                        (upper - proposal_edge_probs) / (upper - edge_threshold),
                        min=0.0,
                        max=1.0,
                    )
                fp_edge_keep_loss = masked_edge_keep_regularization_loss_from_pair_weights(
                    outputs["edge_delta_logits_local"],
                    predicted_only_edge_weights,
                )
                total_loss = local_loss_dict["total_loss"] + fp_edge_keep_loss_weight * fp_edge_keep_loss
            else:
                if fp_keep_lowconf_only:
                    predicted_only_edge_mask = predicted_only_edge_mask * (
                        proposal_edge_probs < fp_keep_lowconf_upper
                    ).float()
            ...
            delete_rescue_loss = outputs["edge_delta_logits_local"].sum() * 0.0
            if use_oracle_delete_rescue:
                if delete_rescue_upper <= 0:
                    delete_rescue_weights = oracle_edge_scope_mask * 0.0
                else:
                    oracle_delete_mask = (
                        oracle_edge_scope_mask
                        * valid_edge_mask
                        * (edge_delta_targets == EDGE_DELTA_DELETE).float()
                    )
                    delete_rescue_weights = oracle_delete_mask * torch.clamp(
                        (delete_rescue_upper - proposal_edge_probs) / delete_rescue_upper,
                        min=0.0,
                        max=1.0,
                    )
                delete_rescue_loss = masked_edge_delete_regularization_loss_from_pair_weights(
                    outputs["edge_delta_logits_local"],
                    delete_rescue_weights,
                )
                total_loss = total_loss + delete_rescue_weight * delete_rescue_loss
            ...
            local_loss_dict["delete_rescue_loss"] = delete_rescue_loss
...
def main() -> None:
    ...
    parser.add_argument(
        "--save_dir",
        type=str,
        default="checkpoints/oracle_local_delta_proposal_conditioned_fp_keep",
    )
    ...
    parser.add_argument("--fp_edge_keep_loss_weight", type=float, default=0.5)
    parser.add_argument("--fp_keep_lowconf_only", action="store_true")
    parser.add_argument("--fp_keep_lowconf_upper", type=float, default=0.35)
    parser.add_argument("--fp_keep_weighted", action="store_true")
    parser.add_argument("--fp_keep_weighted_upper", type=float, default=0.35)
    parser.add_argument("--fp_keep_asymmetric", action="store_true")
    parser.add_argument("--fp_keep_absent_weight", type=float, default=0.0)
    parser.add_argument("--fp_keep_present_weight", type=float, default=0.0)
    parser.add_argument("--use_oracle_delete_rescue", action="store_true")
    parser.add_argument("--delete_rescue_weight", type=float, default=0.0)
    parser.add_argument("--delete_rescue_upper", type=float, default=0.35)
    ...
    print(f"fp-keep lowconf only: {args.fp_keep_lowconf_only}")
    print(f"fp-keep lowconf upper: {args.fp_keep_lowconf_upper}")
    print(f"fp-keep weighted: {args.fp_keep_weighted}")
    print(f"fp-keep weighted upper: {args.fp_keep_weighted_upper}")
    print(f"fp-keep asymmetric: {args.fp_keep_asymmetric}")
    print(f"fp-keep absent weight: {args.fp_keep_absent_weight}")
    print(f"fp-keep present weight: {args.fp_keep_present_weight}")
    print(f"use oracle delete rescue: {args.use_oracle_delete_rescue}")
    print(f"delete rescue weight: {args.delete_rescue_weight}")
    print(f"delete rescue upper: {args.delete_rescue_upper}")
    ...
    for epoch in range(1, args.epochs + 1):
        train_metrics = run_oracle_epoch(
            ...
            fp_keep_lowconf_only=args.fp_keep_lowconf_only,
            fp_keep_lowconf_upper=args.fp_keep_lowconf_upper,
            fp_keep_weighted=args.fp_keep_weighted,
            fp_keep_weighted_upper=args.fp_keep_weighted_upper,
            fp_keep_asymmetric=args.fp_keep_asymmetric,
            fp_keep_absent_weight=args.fp_keep_absent_weight,
            fp_keep_present_weight=args.fp_keep_present_weight,
            use_oracle_delete_rescue=args.use_oracle_delete_rescue,
            delete_rescue_weight=args.delete_rescue_weight,
            delete_rescue_upper=args.delete_rescue_upper,
        ...
        print(
            ...
            f"val_fp_edge_keep_loss={val_metrics['local_fp_edge_keep_loss']:.6f} | "
        ...
        ckpt = {
            ...
            "scope_source": "oracle_supervision_with_proposal_conditioning_plus_predicted_only_edge_keep_regularization",
