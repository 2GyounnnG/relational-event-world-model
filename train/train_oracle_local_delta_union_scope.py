...
#   python train/train_oracle_local_delta_union_scope.py
# ---------------------------------------------------------------------
...
def build_training_scope_masks(
    predicted_scope_nodes: torch.Tensor,
    predicted_scope_edges: torch.Tensor,
    batch: Dict[str, torch.Tensor],
) -> tuple[torch.Tensor, torch.Tensor]:
    oracle_scope_nodes = batch["event_scope_union_nodes"].float()
    oracle_scope_edges = batch["event_scope_union_edges"].float()
    train_scope_nodes = torch.maximum(predicted_scope_nodes, oracle_scope_nodes)
    train_scope_edges = torch.maximum(predicted_scope_edges, oracle_scope_edges)
    return train_scope_nodes, train_scope_edges
...
def run_epoch(
    ...
    with context:
        for batch in loader:
            ...
            predicted_scope_nodes, predicted_scope_edges = predict_scope_masks(
            ...
            if use_union_scope:
                scope_node_mask, scope_edge_mask = build_training_scope_masks(
                    predicted_scope_nodes,
                    predicted_scope_edges,
                    batch,
                )
            else:
                scope_node_mask, scope_edge_mask = predicted_scope_nodes, predicted_scope_edges
...
def main() -> None:
    ...
    parser.add_argument("--save_dir", type=str, default="checkpoints/oracle_local_rewrite_delta_union_scope")
    ...
    if "event_scope_union_nodes" not in sample or "event_scope_union_edges" not in sample:
        raise KeyError(
            "Union-scope training requires event_scope_union_nodes and event_scope_union_edges in each dataset sample."
        )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        ckpt = {
            ...
            "scope_source": "union_scope_train_predicted_scope_eval",
