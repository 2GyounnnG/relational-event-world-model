...
SMALL_GUARD_MODE = "step11_vanilla_guard_product"
SCORES_PROBE_MODE = "step14_scores_only_probe"
ENRICHED_PROBE_MODE = "step14_enriched_local_context_probe"
...
def score_candidates_with_probe(
    ...
    features, _, _, candidates = build_candidate_feature_tensor(
        batch=batch,
    ...
    scores = torch.zeros_like(candidates, dtype=completion_logits.dtype)
    if features.numel() <= 0:
        return scores
    logits = probe_model(features)
    scores[candidates] = torch.sigmoid(logits)
    return scores * base_outputs["valid_edge_mask"].float()
...
def evaluate_representation_probe(
    ...
    scores_probe_model: torch.nn.Module,
    ...
    for batch in loader:
        ...
        reference_guard_probs = torch.sigmoid(reference_guard_logits) * valid_edge_mask.float()
        scores_probe_probs = score_candidates_with_probe(
            probe_model=scores_probe_model,
            feature_bundle=SCORES_ONLY,
            batch=batch,
            base_outputs=base_outputs,
            completion_logits=completion_logits,
            reference_guard_logits=reference_guard_logits,
        ...
        ranking_scores_by_mode = {
            ...
            SMALL_GUARD_MODE: completion_probs * reference_guard_probs,
            SCORES_PROBE_MODE: scores_probe_probs,
            ENRICHED_PROBE_MODE: enriched_probe_probs,
        ...
        mode_outputs: Dict[str, Dict[str, torch.Tensor]] = {
            ...
            SMALL_GUARD_MODE: build_ranked_outputs(
                base_outputs=base_outputs,
                mode_name=SMALL_GUARD_MODE,
                ranking_scores=ranking_scores_by_mode[SMALL_GUARD_MODE],
                completion_probs=completion_probs,
            ),
            SCORES_PROBE_MODE: build_ranked_outputs(
                base_outputs=base_outputs,
                mode_name=SCORES_PROBE_MODE,
                ranking_scores=ranking_scores_by_mode[SCORES_PROBE_MODE],
                completion_probs=completion_probs,
            ),
            ENRICHED_PROBE_MODE: build_ranked_outputs(
                base_outputs=base_outputs,
                mode_name=ENRICHED_PROBE_MODE,
                ranking_scores=ranking_scores_by_mode[ENRICHED_PROBE_MODE],
                completion_probs=completion_probs,
            ),
...
def main() -> None:
    ...
    parser.add_argument("--scores_probe_checkpoint_path", type=str, required=True)
    ...
    parser.add_argument("--artifact_dir", type=str, default="artifacts/step14_guard_representation_probe")
    ...
    scores_probe_checkpoint_path = resolve_path(args.scores_probe_checkpoint_path)
    ...
    scores_probe_model = load_probe_model(scores_probe_checkpoint_path, device)
    ...
    results = evaluate_representation_probe(
        proposal_model=proposal_model,
        completion_model=completion_model,
        reference_guard_model=reference_guard_model,
        scores_probe_model=scores_probe_model,
    ...
    payload = {
        "metadata": {
            ...
            "scores_probe_checkpoint_path": str(scores_probe_checkpoint_path),
