...
from models.encoder_step30 import build_pair_mask, masked_edge_bce_loss
from train.eval_step30_encoder_recovery import load_model as load_step30_model
from train.eval_step31_multi_view_bridge import load_step31_model, simple_late_fusion_outputs
from train.train_step31_multi_view_encoder import get_device, move_batch_to_device
from train.train_step31d_late_fusion_distilled_encoder import (
    build_loader,
    evaluate_recovery,
    freeze_except_edge_head,
    save_checkpoint,
    set_seed,
)


def negative_only_teacher_ceiling_loss(
    ...
    """Recall-preserving teacher loss.

    Late fusion is used only as a ceiling for GT-negative disagreement pairs.
    GT-positive pairs are not pulled down toward the teacher, preserving learned
    fusion's proposal-sensitive recall path.
    """
    ...
    negative_mask = (target_adj < 0.5).float()
    active_mask = pair_mask * gate * negative_mask
    ...
    return (loss * active_mask).sum() / (active_mask.sum() + eps)
...
def loss_for_batch(
    ...
    teacher_loss = negative_only_teacher_ceiling_loss(
...
def run_epoch(
    ...
    teacher_model.eval()
    totals = {"total_loss": 0.0, "edge_loss": 0.0, "teacher_loss": 0.0}
    count = 0
...
def main() -> None:
    ...
    parser.add_argument("--save_dir", default="checkpoints/step31e_recall_preserving_late_fusion_teacher")
    ...
    parser.add_argument("--teacher_loss_weight", type=float, default=0.35)
    parser.add_argument("--teacher_margin", type=float, default=0.25)
    ...
    print("teacher: late-fusion ceiling only for GT-negative disagreement pairs")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        selection_score = (
            ...
            + 0.15 * float(val_recovery["edge_precision"])
            + 0.10 * float(val_recovery["edge_recall"])
            - 0.03 * float(val_loss["teacher_loss"])
        )
        val_metrics = {
