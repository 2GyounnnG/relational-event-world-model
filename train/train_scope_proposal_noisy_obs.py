...
def compute_prf_counts(
    ...
    target = targets.float() * valid_mask
...
def finalize_epoch_metrics(metric_sums: Dict[str, float], num_batches: int) -> Dict[str, float]:
...
def run_epoch(
    ...
    with context:
        for batch in loader:
            ...
            model_node_feats, model_adj = get_model_inputs(batch)
    ...
    return finalize_epoch_metrics(metric_sums, num_batches)
...
def main() -> None:
    ...
    parser.add_argument("--train_path", type=str, default="data/graph_event_step6a_train.pkl")
    parser.add_argument("--val_path", type=str, default="data/graph_event_step6a_val.pkl")
    ...
    parser.add_argument("--edge_scope_pos_weight", type=float, required=True)
    ...
    if "event_scope_union_nodes" not in sample or "event_scope_union_edges" not in sample:
        raise KeyError("Noisy proposal training still requires clean oracle scope labels.")
    ...
    print("proposal input mode: obs_node_feats/obs_adj when present, otherwise clean node_feats/adj")
    print(
        f"weights: node_flip_weight={args.node_flip_weight}, "
        f"edge_scope_pos_weight={args.edge_scope_pos_weight}"
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        ckpt = {
            ...
            "input_mode": "noisy_structured_observation_if_available",
        }
