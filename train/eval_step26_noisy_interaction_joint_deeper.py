...
def main() -> None:
    ...
    parser.add_argument("--step25_artifact_dir", type=str, default="artifacts/step25_noisy_multievent_oracle_headroom")
    parser.add_argument("--step26_artifact_dir", type=str, default="artifacts/step26_noisy_interaction_joint_deeper")
    parser.add_argument("--step26_run_name", type=str, default="noisy_step26_joint_deeper")
    ...
    step25_dir = resolve_path(args.step25_artifact_dir)
    ...
    run_paths = {
        ...
        "step26_deeper_joint": step26_dir / f"{args.step26_run_name}.json",
        "oracle_rft1_reference_optional": step25_dir / "oracle_scope_rft1.json",
        "oracle_w012_reference_optional": step25_dir / "oracle_scope_w012.json",
    }
    ...
    comparisons: Dict[str, Any] = {}
    candidate = runs.get("step26_deeper_joint")
    add_comparison(comparisons, "step26_minus_baseline_noisy_p2_rft1", candidate, runs.get("baseline_noisy_p2_rft1"))
    add_comparison(comparisons, "step26_minus_step23_proposal_only", candidate, runs.get("step23_proposal_only_rft1"))
    add_comparison(comparisons, "step26_minus_step24_light_joint", candidate, runs.get("step24_light_joint"))
    add_comparison(comparisons, "step26_minus_noisy_p2_i1520_optional", candidate, runs.get("baseline_noisy_p2_i1520_optional"))

    summary = {
        "metadata": {
            "evaluation": "Step 26 coverage-emphasized joint noisy interaction comparison",
            "single_mechanism_variable": "joint_recipe = light_joint_baseline | coverage_emphasized_joint",
            "fixed_thresholds": {"node_threshold": 0.15, "edge_threshold": 0.10},
            "recipe_note": (
                "Step26 uses stronger proposal coverage pressure inside Step24-style joint training: "
                "joint_proposal_loss_weight=2.0, edge_scope_pos_weight=4.0, and coverage-aware checkpoint selection."
            ),
            "step22_artifact_dir": str(step22_dir),
            "step23_artifact_dir": str(step23_dir),
            "step24_artifact_dir": str(step24_dir),
            "step25_artifact_dir": str(step25_dir),
            "step26_artifact_dir": str(step26_dir),
        },
    ...
    out_json = step26_dir / f"{args.output_name}.json"
    out_csv = step26_dir / f"{args.output_name}.csv"
