...
STEP11_MODE = "step11_vanilla_guard_product"
STEP13_MODE = "step13_tail_focused_guard_product"
ORACLE_MODE = "oracle_event_scope_guard_budget_0.10"
NAIVE_MODE = "naive_induced_closure_reference"
MODE_ORDER = [BASE_MODE, STEP9C_MODE, STEP11_MODE, STEP13_MODE, ORACLE_MODE, NAIVE_MODE]
RESCUE_MODES = [STEP9C_MODE, STEP11_MODE, STEP13_MODE, ORACLE_MODE]


@torch.no_grad()
def evaluate_tailfocused_guard(
    proposal_model: torch.nn.Module,
    completion_model: GatedInternalEdgeCompletionHead,
    vanilla_guard_model: torch.nn.Module,
    tail_guard_model: torch.nn.Module,
    ...
    vanilla_guard_model.eval()
    tail_guard_model.eval()
    rewrite_model.eval()
    ...
    for batch in loader:
        ...
        vanilla_guard_probs = torch.sigmoid(
            vanilla_guard_model(
                node_latents=base_outputs["node_latents"],
                base_edge_logits=base_outputs["edge_scope_logits"],
        ...
        tail_guard_probs = torch.sigmoid(
            tail_guard_model(
                node_latents=base_outputs["node_latents"],
                base_edge_logits=base_outputs["edge_scope_logits"],
                completion_logits=completion_logits,
        ...
        ranking_scores_by_mode = {
            ...
            STEP11_MODE: completion_probs * vanilla_guard_probs,
            STEP13_MODE: completion_probs * tail_guard_probs,
        ...
        mode_outputs: Dict[str, Dict[str, torch.Tensor]] = {
            ...
            STEP11_MODE: build_ranked_outputs(
                base_outputs=base_outputs,
                mode_name=STEP11_MODE,
                ranking_scores=ranking_scores_by_mode[STEP11_MODE],
                completion_probs=completion_probs,
            ),
            STEP13_MODE: build_ranked_outputs(
                base_outputs=base_outputs,
                mode_name=STEP13_MODE,
                ranking_scores=ranking_scores_by_mode[STEP13_MODE],
                completion_probs=completion_probs,
            ),
...
def main() -> None:
    ...
    parser.add_argument("--vanilla_guard_checkpoint_path", type=str, required=True)
    parser.add_argument("--tail_guard_checkpoint_path", type=str, required=True)
    parser.add_argument("--rewrite_checkpoint_path", type=str, required=True)
    parser.add_argument("--data_path", type=str, required=True)
    parser.add_argument("--split_name", type=str, default="test")
    parser.add_argument("--run_name", type=str, default="")
    parser.add_argument("--artifact_dir", type=str, default="artifacts/step13_tailfocused_scope_guard")
    ...
    vanilla_guard_checkpoint_path = resolve_path(args.vanilla_guard_checkpoint_path)
    tail_guard_checkpoint_path = resolve_path(args.tail_guard_checkpoint_path)
    ...
    vanilla_guard_model = load_guard_model(vanilla_guard_checkpoint_path, device)
    tail_guard_model = load_guard_model(tail_guard_checkpoint_path, device)
    rewrite_model, use_proposal_conditioning = load_rewrite_model(rewrite_checkpoint_path, device)

    results = evaluate_tailfocused_guard(
        proposal_model=proposal_model,
        completion_model=completion_model,
        vanilla_guard_model=vanilla_guard_model,
        tail_guard_model=tail_guard_model,
    ...
    payload = {
        "metadata": {
            ...
            "vanilla_guard_checkpoint_path": str(vanilla_guard_checkpoint_path),
            "tail_guard_checkpoint_path": str(tail_guard_checkpoint_path),
            ...
            "guard_objective": "tail_focused_hard_negative_scope_bce",
